"""Automated Setup for Web3 Interaction with Local Node
"""
...
def web3_geth_setup():
    """Gets a Web3 session with a local Geth node on the default port

    Returns:
        Web3: a web3 session object
    """
...
def compile_contracts(w3, fname):
    """Compiles contracts in a file

    Args:
        w3 (Web3): a web3 session
        fname (str): path to a solidity file 

    Returns:
        dict: A mapping of ID's to w3.eth.contract objects 
    """
...
def constructZilliqaToken(w3, contracts, instrumented=False):
    """Helper / example method to construct the ZilliqaToken contract"""
...
def constructBuchiChecker(w3, contract):
    """Helper / example method to construct the monitor contract"""
