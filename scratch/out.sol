...
bool temp_ret_instrum_0 = super.transfer(_to, _value);
bc.apply_updates();
bc.check();
return temp_ret_instrum_0;
    ...
    {
BuchiChecker bc = BuchiChecker(buchi_checker_address);
bool temp_ret_instrum_0 = super.transferFrom(_from, _to, _value);
bc.apply_updates();
bc.check();
return temp_ret_instrum_0;
    }
...
bool temp_ret_instrum_0 = burn(_value);
bc.apply_updates();
bc.check();
return temp_ret_instrum_0;
    }
...
function initialize(address a) public {
...
contract Deployer {
	ZilliqaToken t;
	constructor() public {
		t = new ZilliqaToken(0xDEADBEEF, 1000);
	}
}
...
contract BuchiChecker {
        ...
        function update(uint32 k, bool v) public {
                updates_k.push(k);
                updates_v.push(v);
        }

        function apply_updates() public {
        ...
        function sum(uint32[]) public pure returns (uint32) {
            return 0;
        }
...
contract TestZilliqaToken is ZilliqaToken {
    constructor() public {
	...
	admin = msg.sender;
        ...
        emit Transfer(address(0x0), msg.sender, 1000);
	this.initialize(address(bc));
}
