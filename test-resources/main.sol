...
contract ZilliqaToken is PausableToken {
    ...
    function ZilliqaToken( address _admin, uint _totalTokenAmount, address _verifier ) 
    {
    ...
    {
	BuchiChecker bc = BuchiChecker(verifier);
	bc.update(1, true);
	bc.update(1, true);
        bool retval = super.transfer(_to, _value);
...
contract BuchiChecker {
	...
	function check() {
		if (state == 0) {
			...
			if (vars[2] || vars[1]) {
				state = 2;
				revert("Illegal Buchi Automate State");
			}
		}
	}
}
